"""
Lambda function for GET /trending-queries/{date} endpoint
Retrieves historical trending queries by date from DynamoDB
"""
import json
import os
import logging
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    Lambda handler for GET /trending-queries/{date} endpoint
    ...
    Returns:
        JSON response with trending queries for the specified date
    """
    try:
        ...
        xray_recorder.put_annotation('query_date', date)
        
        logger.info(f"Querying trending queries for date: {date}, limit: {limit}")
        
        # Query DynamoDB for trending queries
        xray_recorder.begin_subsegment('query_dynamodb')
        response = _query_trending_queries(date, limit, next_token)
        xray_recorder.put_metadata('items_returned', len(response.get('Items', [])))
        xray_recorder.end_subsegment()
        
        # Check if any results were found
        if not response.get("Items"):
            logger.info(f"No trending queries found for date: {date}")
            return {
                "statusCode": 404,
                "headers": {
                ...
                "body": json.dumps({
                    "error": "NotFound",
                    "message": f"[NotFound] No trending queries found for date: {date}"
                })
            }
        
        # Format response
        xray_recorder.begin_subsegment('format_response')
        formatted_response = _format_response(response, date)
        xray_recorder.end_subsegment()
...
def _parse_date(event: Dict[str, Any]) -> str:
    """
    Parse and validate the date path parameter
    ...
    Raises:
        ValueError: If date is invalid or missing
    """
    # Try to get date from different possible locations in the event
    date = event.get("date")
    
    if not date:
        path_params = event.get("pathParameters", {})
        date = path_params.get("date") if path_params else None
...
def _query_trending_queries(
    ...
    """
    Query DynamoDB for trending queries by date
